...
long solve_2d_linearconv_pyccel(int n0_u, int n1_u, double *u, int n0_un, int n1_un, double *un, long nt, long dt, double dx, double dy, double c);
...
PyObject *solve_2d_linearconv_pyccel_wrapper(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O!O!llddd", kwlist, &PyArray_Type, &u, &PyArray_Type, &un, &nt, &dt, &dx, &dy, &c))
    {
        return NULL;
    }
    if (PyArray_NDIM(u) != 2)
...
static PyMethodDef mod_l7rwzmku_methods[] = {
    {
...
static struct PyModuleDef mod_l7rwzmku_module = {
    ...
    -1,
    mod_l7rwzmku_methods
};

/*........................................*/

PyMODINIT_FUNC PyInit_mod_l7rwzmku(void)
{
    PyObject *m;

    import_array();

    m = PyModule_Create(&mod_l7rwzmku_module);
